...
const getUserPosts = async (req, res, next) => {
    try {
        const { id } = req.params;
        const posts = await Post.find({ creators: id }).sort({ createdAt: -1 })
        res.status(200).json(posts)
    } catch (error) {
        return next(new HttpError(error))
    }
...
const editPost = async (req, res, next) => {
    try {
        ...
        if (!req.files) {
            ...
            const { thumbnail } = req.files;
            if (thumbnail.size > 2000000) {
                return next(new HttpError("lamba hai ri madam , lamba .. teri jaan kasam hai lamba", 422))
            }
        ...
        if (req.user.id == post.creator) {
            fs.unlink(path.join(__dirname, '..', 'uploads', fileName), async (err) => {
                if (err) {
                    ...
                    await Post.findByAndDelete(postId);
                    const currentUser = await User.findById(req.user.id);
                    const userPostCount = currentUser?.posts - 1;
                    await User.findByIdAndUpdate(req.user.id, { posts: userPostCount })
            ...
            return next(new HttpError("Post not deleted", 403))
        }
